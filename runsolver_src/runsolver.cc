...
private:
  ...
  static void watcherSigHandler(int signum, siginfo_t *siginfo, void *ucontext)
  {
    ...
    if (signum==SIGTERM || signum==SIGINT)
    {
      ...
      instance->abortChild=true;
      return;
    }
  }

  /**
   * function that starts the timer thread
   */
  static void *startTimerThread(void *)
  {
    instance->runTimerThread();
  ...
  void runTimerThread()
  {
    ...
    // the program we started has terminated but some of its
...
public:
  ...
  void run(char **cmd)
  {
    ...
    */
	...
	int err=pthread_create(&timerThreadTID,NULL,startTimerThread,NULL);
	if (err)
